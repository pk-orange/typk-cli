...
function normalizeConfig(userConfig) {
  ...
  if (typeof userConfig.dimUntyped === "boolean") {
    config.dimUntyped = userConfig.dimUntyped;
  }
  ...
  if (Number.isFinite(userConfig.commandsPerScreen)) {
    const rounded = Math.round(userConfig.commandsPerScreen);
    config.commandsPerScreen = Math.max(1, rounded);
  }

  if (typeof userConfig.displayTooltips === "boolean") {
    config.displayTooltips = userConfig.displayTooltips;
  }

  if (typeof userConfig.displayPauseTooltip === "boolean") {
    config.displayPauseTooltip = userConfig.displayPauseTooltip;
  }
